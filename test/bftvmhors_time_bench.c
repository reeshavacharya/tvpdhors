...
int main(int argc, char **argv) {
    ...
    bftvmhors_hp_t hp;
    if (bftvmhors_new_hp(&hp, (const u8*)config_path) != BFTVMHORS_NEW_HP_SUCCESS) {
    ...
    bftvmhors_signer_t signer; bftvmhors_new_signer(&signer, &hp, &keys);
    bftvmhors_signature_t sig; sig.signature = malloc(signer.hp->k * BITS_2_BYTES(signer.hp->l));

    long long *t_sign = malloc(sizeof(long long)*iters);
    for (int i=0;i<iters;i++) {
        struct timespec t0, t1; clock_gettime(CLOCK_MONOTONIC, &t0);
        bftvmhors_sign(&sig, &signer, message, message_len);
        clock_gettime(CLOCK_MONOTONIC, &t1);
    ...
    for (int i=0;i<iters;i++) {
        struct timespec t0, t1; clock_gettime(CLOCK_MONOTONIC, &t0);
        bftvmhors_verify(&verifier, &hp, &sig, message, message_len);
        clock_gettime(CLOCK_MONOTONIC, &t1);
    ...
    printf("BFTVMHORS Benchmark (ITER=%d)\n", iters);
    ...
    free(sig.signature); bftvmhors_destroy_keys(&keys); bftvmhors_destroy_hp(&hp); free(message);
    return 0;
}
